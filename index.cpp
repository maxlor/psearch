...
Index::Index(const string filename, bool need_descpath, bool need_categories) 
	: file(filename.c_str(), ifstream::in)
{
	this->need_descpath = need_descpath | need_categories;
	this->need_categories = need_categories;
	
	fields_len = (need_categories ? 7 : (need_descpath ? 5 : 4));
}
...
void Index::print_line(bool flag_name, bool flag_long) {
